	...
	export default {
		...
		computed: {
			percentage() {
				if (this.imageIds.length !== 0) {
					return parseInt((this.loadedNum / this.imageIds.length).toFixed(2) * 100);
				} else {
					return 0;
				}
			}
		...
		watch: {
			...
			flip: {
				...
				handler(val) {
					...
					viewport.hflip = val.hflip;
					viewport.vflip = val.vflip;
			...
			currentImageIndex(val) {
				...
				if (val > this.imageIds.length - 1) {
					this.currentImageIndex = 0;
					return false;
				}
				if (val < 0) {
					this.currentImageIndex = this.imageIds.length - 1;
					return false;
				}
				// todo 未成功加载的帧数重新加载
		...
		methods: {
			...
			getLesionIndexByCoordZ(index) {
				if (this.imgSort === 'top') {
					index = this.imageIds.length - index - 1;
				}
				return index;
			},
			initImageIds() {
