	...
	<section class="dn-pkg-dicom-viewer">
		...
		>
			<div class="canvasContainer">
				<div class="canvas dicom-canvas-box" ref="dicomImage"></div>
				<!--右键菜单栏-->
			...
			></scroll-bar>
			<!--标注结节的弹框-->
			<div class="mark-dialog" :style="markDialogStyle" v-if="showMarkDialog && markNoduleOn">
				<span style="color: #fff;">结节类型</span>
				<el-select v-model="markNoduleInfo.lesion.feature_label">
					<el-option
						v-for="(item, key) in propType"
						:key="key"
						:label="item"
						:value="key"
					></el-option>
				...
				<div class="btns">
					<el-button class="btn" size="mini" @click="cancelAddLesion">
						取消
					</el-button>
					<el-button class="btn" size="mini" @click="confirmAddLesion">
						确认
					</el-button>
				</div>
			</div>
...
import { noduleMark } from './customTools/markNodule';
...
export default {
	...
	watch: {
		...
		currentImageIndex(val, oldVal) {
			...
			if (this.isCached(val) || this.dicomSource === 'local') {
				// 判断当前帧数里是否有一个以上结节，如果有则indexofCurrentZ就初始化0；
				// 如果没有则设为-1
				this.renderImage(val); //渲染图片
				this.updateCurrentLesionInfo(val);
			} else {
				this.$nextTick(() => {
					this.currentImageIndex = oldVal;
					this.$emit('message', 'notLoaded');
				});
			}
	...
	methods: {
		...
		renderFirstImage() {
			...
			if (FIRST_NODULE_INDEX === 0) {
				this.renderImage(FIRST_NODULE_INDEX).then(() => {
		...
		renderNetImage(index) {
			...
			return cornerstone.loadAndCacheImage(this.imageIds[imageIndex]).then((image) => {
				...
				cornerstone.displayImage(this.element, image, viewport);
				if (!this.ww) {
					this.setWindow(this.$store.state.windowSetting.ww, this.$store.state.windowSetting.wl);
				} else {
					this.setWindow(this.ww, this.wl);
				}
			});
		...
		getImages(ids) {
			...
			for (let i of ids) {
				...
				// 为每个请求添加30s的超时处理
				timers[i] = setTimeout(() => {
					this.$message.warning(`第${this.imageIds.indexOf(i) + 1}帧未加载成功`);
				}, 30 * 1000);
				cornerstone
					.loadAndCacheImage(i)
					.then(() => {
					...
					.catch(() => {
						this.loadErrorFrame[i] = 'notReload';
					})
					.finally(() => {
		...
		imageRendered(e) {
			...
			this.wl = Math.round(viewport.voi.windowCenter);
...
<style lang="scss" src="../../style/dicomViewer.scss"></style>
